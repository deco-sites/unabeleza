...
export default function PaymentMethods(
  { methods, priceCurrency }: PaymentMethodsProps,
) {
  const credit = {
  ...
  const paymentMethods: Method[] = methods.filter((obj) =>
    obj.name && obj.name !== "Cartão de Crédito"
  );
                  ...
                  {obj.installments.map((item) => (
                    <li class="flex justify-between odd:bg-white even:bg-zinc-200 px-5 py-4 mobile:px-4 mobile:py-3">
                      <span class="mobile:text-xs">
                        ...
                        {formatPrice(item.price, priceCurrency)}
                      </span>
                      <span class="mobile:text-xs">
                        {formatPrice(
                          item.billingDuration * item.price,
                          priceCurrency,
                        )}
                      </span>
                    </li>
          ...
          <>
            ...
            <div role="tabpanel" class="tab-content p-10 mobile:p-5">
              <div class="flex flex-col gap-5">
                <span class="font-bold text-black mobile:text-xs">
                  {formatPrice(obj.price, priceCurrency)} no {obj.name}
                </span>
                {obj.name === "Pix"
                  ? (
                    <span class="mobile:text-xs">
                      O pagamento é instantâneo e só pode ser à vista. Na etapa
                      de finalização da compra, a gente explica direitinho como
                      pagar com Pix.
                    </span>
                  )
                  : obj.name === "Boleto" && (
