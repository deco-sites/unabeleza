...
export const action = async (
  ...
  if (!user) {
    return { ...props, state: "error", message: "Usuário não autenticado" };
  }
...
export default function ReviewForm(
  { productVariantId, state, message, reviewCount, ratingValue }:
    ReviewFormProps,
) {
    ...
    <div class="grid grid-cols-1 gap-[26px] desktop:w-[29.24%] w-full">
      ...
      >
          ...
          <div class="flex flex-col gap-2">
            <div className="rating">
              <input
                ...
                name="stars"
                value="1"
                className="mask mask-star-2 bg-primary h-4 w-4 mobile:h-6 mobile:w-6"
              />
              <input
                type="radio"
                name="stars"
            ...
            <span class="text-xs text-[#373737]">
        ...
        <textarea
          ...
          maxlength={300}
        />
        <button class="btn btn-primary text-black text-sm font-bold rounded-[5px]">
          Enviar Avaliação
        </button>
      </form>
    </div>
