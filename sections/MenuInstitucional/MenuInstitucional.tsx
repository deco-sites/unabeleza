...
export default function MenuInstitucional(
  ...
  return (
    <div class="hidden desktop:block absolute top-[280px] left-[80px] list-none bg-[#FAFAFA] py-[40px] pl-[40px] pr-[49px]">
      <h3 className="text-slate-950 text-[16px] font-bold mb-[20px]">
      ...
      <div>
        {links_institucionais &&
          links_institucionais.map(({ rota, etiqueta }, index) => (
            <li
              className="text-slate-950 text-[12px] font-regular hover:text-[#8F2AED] cursor-pointer"
              key={index}
            >
              <a href={rota}>{etiqueta}</a>
            </li>
          ))}
      </div>
      ...
      <div>
        {links_ajudas &&
          links_ajudas.map(({ rota, etiqueta }, index) => (
            <li
              className="text-slate-950 text-[12px] font-regular hover:text-[#8F2AED]"
              key={index}
            >
              <a href={rota}>{etiqueta}</a>
            </li>
